...
%#########################################################################
Meas.t = linspace(Tstart, Tstart+Length*Tinterval, Length);  %Zeitvektor

%Messdaten
Meas.V_out = B * scale.Vout.factor; % Gemessene Ausgangsspannung [V]
Meas.I_L   = (C+scale.I_L.offset) * scale.I_L.factor; % Gemessener Spulenstrom [A]
...
param.R_load = param.R_load/3;  %Lastwiderstand [Ohm] 

D_pre  = 0.3;  %Initialer Duty Cycle [-]
D_post = 0.5;  %Duty Cycle nach Sprung [-]

% Ideales Modell
SimIdeal.x0 = [param.V_ls / ((1 - D_pre)^2 * param.R_load); param.V_ls / (1 - D_pre)]; %Anfangswert

%Simulation
options = odeset('RelTol',1e-8);
tspan   = [Tstart, Tstart+Length*Tinterval];    %Simulationszeit
...
T_step = 0;  %Zeitpunkt des Sprungs im Duty Cycle
sim('Boost.slx')
